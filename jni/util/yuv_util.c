...
unsigned char* rotateYUV420SPDegree270(jbyte *yuvInBuffer, jint inWidth, jint inHeight,
		jint* outWidth, jint* outHeight) {
	D("rotate 270 degree");
	...
	*outHeight = inWidth;
	...
	unsigned char *outYuv = (unsigned char *)malloc(sizeof(unsigned char) * (int)((3 * size) * 0.5));
...
//			D("rotating Y - oY[%d, %d] = iY[%d, %d]", i, j, inHeight - 1 - j, i);
	...
	printf("rotate C: out C width: %d, out C height: %d, inCWidth: %d, inCHeight: %d\n", outCWidth, outCHeight, inCWidth, inCHeight);
	for (i = 0; i < outCHeight; i++) {
		for (j = 0; j < outCWidth; j += 2) {
//			printf("loop - i: %d, j: %d\n", i, j);
			int inI = (int)(j * 0.5);
...
unsigned char *rotateYUV420SP(jbyte *yuvInBuffer, jint inWidth, jint inHeight, ROTATE_DEGREE degree, jint *outWidth, jint *outHeight) {
	...
	switch(degree) {
	case ROTATE_90:
		outYuv = rotateYUV420SPDegree90(yuvInBuffer, inWidth, inHeight, outWidth, outHeight);
		break;
	case ROTATE_270:
		outYuv = rotateYUV420SPDegree270(yuvInBuffer, inWidth, inHeight, outWidth, outHeight);
		break;
	}
	return outYuv;
}
